...
def plot_batch_means_over_time(
    transient_data: Dict[str, Any], 
    t_star_k: Optional[int] = None,
    moving_avg_window: Optional[int] = 20,
    ax: Optional[matplotlib.axes.Axes] = None
) -> matplotlib.axes.Axes:
    """
    Generates a Time Series Plot of Batch Means and its Moving Average.

    This function takes pre-calculated transient data and plots the
    evolution of the KPI over time. It highlights the transient
    phase with a grey background.

    Args:
        transient_data (Dict[str, Any]): The data dictionary produced
            by calculate_transient_data().
        t_star_k (Optional[int]): The estimated transient end point
            (as a batch index 'k'). This is used to draw the
            transient background.
        moving_avg_window (Optional[int]): The window size (in number of
            batches) for the moving average. If None, it is not plotted.
        ax (Optional[matplotlib.axes.Axes]): The matplotlib Axes
    ...
    # Data Extraction
    try:
        batch_means = transient_data['batch_means']
        k_indices = transient_data['k_indices']
        global_mean = transient_data['global_mean']
        b = len(k_indices)
    except (KeyError, TypeError):
        log.error("Invalid transient_data dictionary passed.")
        ax.set_title("Time Series Plot (Invalid Data)")
        return ax

    # Plotting the Time Series (Batch Means)
    ax.plot(
        k_indices, 
        batch_means, 
        color='grey',
        alpha=0.4, 
        lw=1.0, 
        label="Batch Means (Smoothed Data)"
    )
    
    # Plotting the Moving Average
    if moving_avg_window is not None and moving_avg_window > 0:
        ma_series = pd.Series(batch_means).rolling(
    ...
    # Highlight Transient Phase
    if t_star_k is not None and t_star_k > 0:
        # Add a grey background span from k=0 to k=t_star_k
        ax.axvspan(
            0, 
            t_star_k, 
            color='grey', 
            alpha=0.2, 
            label=f"Transient Phase (k* = {t_star_k})"
        )
        
        # Plot the steady-state mean (calculated *after* t_star_k)
        if t_star_k < b:
            # We can get this value directly from the xk_bar array
            steady_state_mean = transient_data['xk_bar'][t_star_k]
            ax.axhline(
                steady_state_mean,
                color='red',
                linestyle='--',
                lw=2.0,
                label=f"Steady-State Mean = {steady_state_mean:.3f}"
            )

    # Final Layout
    ax.set_title(f"KPI Evolution (Moving Average over Time)")
    ax.set_xlabel(f"Batch Index (as proxy for Time) [0 to {b-1}]")
    ax.set_ylabel(f"Mean KPI Value")
    ax.legend(loc='best')
    ax.set_xlim(left=0)
...
def plot_transient_analysis(
    ...
    """
    Generates a simplified transient convergence plot showing only
    the Relative Error (Rk).

    This plot is used to visually confirm that the relative error
    of the system converges to zero as the transient phase is
    removed.
    ...
    try:
        ...
        log.error("Invalid transient_data dictionary passed.")
        ax.set_title("Relative Error Plot (Invalid Data)")
        return ax

    # Plotting Rk (Relative Error)
    ax.plot(
        k_indices, 
        Rk, 
        color='tab:green', 
        lw=2.0, 
        label="Relative Error ($R_k$)"
    )
    
    # Plotting Reference Lines
    # The reference for Rk is always 0.0
    ax.axhline(
        0, 
        color='black', 
        linestyle=':', 
        lw=1.5,
        label="Steady-State (Error = 0.0)"
    )
    ...
    # Final Layout
    ax.set_title("Transient Analysis (Relative Error Convergence)")
    ax.set_xlabel(f"Initial Batches Removed (k) [out of {b} total]")
    ax.set_ylabel("Relative Error (from Global Mean)")
    ax.tick_params(axis='y')
    ax.set_xlim(left=0, right=b - 1)
